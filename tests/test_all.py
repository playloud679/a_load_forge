...
def _check_dccav_deep_max_extension_finds_deep_optimum():
    ts = _acoustics.get_driver_preset("SBL: HiVi B3N")
    opt = _acoustics.optimize_alignment(
        ts,
        _acoustics.OptimizationGoals(objective="extension", max_ripple_db=3.0),
        load_type="DCCAV",
...
test(
    "DCCAV deep Max extension escapes local traps and beats shallow minima",
    _check_dccav_deep_max_extension_finds_deep_optimum,
)
...
def _check_ui_streamlit_cloud_bounds_processes_and_falls_back_fast():
    ...
    class DeniedProcessPool:
        def __init__(self, *args, **kwargs):
            raise PermissionError("process startup denied")
...
from test_usage_analytics import (
    ...
    check_self_registered_admin_email_is_not_admin,
...
test("UI a self-registered copy of the admin email is not admin", check_self_registered_admin_email_is_not_admin, group="ui")
