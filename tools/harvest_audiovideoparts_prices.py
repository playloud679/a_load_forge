...
def fetch(url: str, timeout: float) -> str:
    req = Request(
        url,
        headers={
            "User-Agent": "LoadForge-PriceCrawler/1.0 (+https://github.com/playloud679/a_load_forge)"
        },
    )
...
def product(html: str, url: str) -> dict | None:
    ...
    for node in soup.select('script[type="application/ld+json"]'):
        try:
            rows.append(json.loads(node.string or node.get_text()))
        except Exception:
            pass
    items: list = []
    for x in rows:
    ...
    data = next(
        (x for x in items if isinstance(x, dict) and (x.get("@type") == "Product" or "Product" in (x.get("@type") or []))),
        None,
    )
    if not data:
        ...
        if not amount or not amount.get("content"):
            return None
        try:
            price = float(str(amount["content"]).replace(",", "."))
        except ValueError:
            return None
        return {
            "name": title.get_text(" ", strip=True) if title else url.rsplit("/", 1)[-1],
            "sku": "",
            "mpn": "",
            "price": price,
            "currency": "EUR",
            "url": url,
            "seller": "Audio Video Parts",
            "source": "Audio Video Parts price crawler",
        }
    ...
    return {
        ...
        "sku": str(data.get("sku") or data.get("mpn") or "").strip(),
...
def main() -> int:
    ...
    ap.add_argument("--checkpoint", type=Path, default=ROOT / "data/audiovideoparts_price_checkpoint.json")
    ...
    if args.checkpoint.exists():
        state.update(json.loads(args.checkpoint.read_text()))
    ...
    for _ in range(args.max_pages):
        ...
        try:
            ...
            rec = product(html, url)
            if rec and not any(x.get("url") == url for x in state["prices"]):
                state["prices"].append(rec)
            new_links = links(html, url)
            new_links.sort(
                key=lambda u: (0 if (u.endswith(".html") or re.search(r"/ecommerce/en/[^/]+-[0-9]+\.html$", u)) else 1, len(u))
            )
            for u in new_links:
                if u not in seen and u not in queue and ("?" not in u or "?p=" in u):
                    queue.append(u)
        except Exception as e:
            state["failures"].append({"url": url, "error": str(e)})
        state.update(queue=queue, visited=sorted(seen))
        args.checkpoint.parent.mkdir(parents=True, exist_ok=True)
        args.checkpoint.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"visited={len(seen)} queue={len(queue)} prices={len(state['prices'])}", flush=True)
