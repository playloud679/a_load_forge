...
catalog_paths = [ROOT / "data/catalog_proprietario.json", ROOT / "data/catalog_vituixcad.json", ROOT / "data/catalog_lsdb.json", ROOT / "data/catalog_speakerboxlite.json"]
...
for catalog_path in catalog_paths:
    ...
    for rec in records:
        url = rec.get("url")
        if not url or not rec.get("price"): continue
        matches = []
        ...
        if not matches:
            ...
            for item in items:
                if any(any(len(k) >= 5 and (k in r or r in k) for k in variants(item.get(field)) for r in rv) for field in ("model", "matched_mpn")):
                    matches.append(item)
        for existing in {id(x): x for x in matches}.values():
            existing["price_url"] = url; existing["price_source"] = "Audio Video Parts"
            if not existing.get("price"):
                existing["price"] = rec["price"]; existing["price_currency"] = rec.get("currency", "EUR"); updated += 1
            added += 1
