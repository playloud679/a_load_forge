...
| `polygonal_horn.py` | [polygonal_horn.md](polygonal_horn.md) | Polygonal N‑gon section engine (area‑matched to circular equivalent), optional rounded corners (`corner_radius`) |
...
| `save_load.py` | [save_load.md](save_load.md) | `.flr` JSON parameter persistence with version metadata for UI presets |
...
- Inward roll-back mouth flanges are geometry-detected and available for circular, polygonal, rectangular, and elliptical sections when the returning lip has enough depth. They use a flat land on the cavity-facing side of the real rim: the outer contour stays at the rim and the inner contour is inset by the requested `Offset from flare`, so the flange never grows outside the flare envelope. Full load-bearing pillars are boolean-clipped to the flare surface before union. The external flare cut is shaft-diameter only. Screw-head seats are cut after the pillar union, stay axial and concentric with the vertical screw holes, and share one coplanar floor controlled by `Head depth`; each section shape places bolts and pillars on its actual rim.
