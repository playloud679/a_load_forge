...
- Flange **DXF** drilling templates (`dxf_export.mesh_to_flange_dxf`) are derived from the flange *mesh* (a planar section through the plate), not from UI parameters — so every flange type exports without duplicated logic. Off-axis loops are bolt holes (emitted as exact nominal circles via the circumscribed vertex radius); the loop centred on the axis is the bore (kept as its true shape). Keep this mesh-derived convention rather than reintroducing per-flange parameter plumbing.
